...
def notification_test(message, title, level):
    ...
    if level == 'info':
        success = notifier.send_info_notification(title, message)
    elif level == 'warning':
        success = notifier.send_warning_notification(title, message)
    else:
        success = notifier.send_error_notification(title, message)
    
    if success:
        click.echo(f"✅ Test notification sent: {title}")
    else:
        click.echo(f"❌ Failed to send notification", err=True)
        sys.exit(1)
...
def notification_services():
    ...
    notifier = HANotifier()
    services = notifier.get_available_services()
    
    if services:
        click.echo("Available notification services:")
        for service in services:
            click.echo(f"  - {service}")
    else:
        click.echo("No notification services found or unable to connect")
